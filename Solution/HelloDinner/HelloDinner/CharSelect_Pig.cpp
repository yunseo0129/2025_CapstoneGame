...
CCharSelect_Pig::CCharSelect_Pig(EngineContext* pContext)
    : CGameObject(pContext)
{
}

CCharSelect_Pig::CCharSelect_Pig(const CCharSelect_Pig& Prototype)
    : CGameObject(Prototype)
{
}

HRESULT CCharSelect_Pig::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CCharSelect_Pig::Initialize(void* pArg)
{
    if (nullptr == pArg)
        return E_FAIL;

    CHARSELECT_PIG_DESC* pDesc = static_cast<CHARSELECT_PIG_DESC*>(pArg);
    ...
    // 상/하체 분리 애니메이션 셋업 (Pig 모델은 상하체 분리 구조)
...
void CCharSelect_Pig::Priority_Update(_float fTimeDelta)
{
}

void CCharSelect_Pig::Update(_float fTimeDelta)
{
...
void CCharSelect_Pig::Late_Update(_float fTimeDelta)
{
...
void CCharSelect_Pig::Render(ID3D12GraphicsCommandList* _commandList)
{
...
void CCharSelect_Pig::ShadowRender(ID3D12GraphicsCommandList* _commandList)
{
    XMFLOAT4X4 WorldMatrix;
    XMStoreFloat4x4(&WorldMatrix, m_pTransformCom->Get_WorldMatrix());
    _commandList->SetGraphicsRoot32BitConstants(RootParameterIndex::GameObject, 16, &WorldMatrix, 0);
...
HRESULT CCharSelect_Pig::Ready_Components()
{
    // 애니메이션 모델 → Clone(깊은 복사)로 인스턴스 전용 뼈 상태 확보
    if (FAILED(Add_Component(m_iModelLevelIndex, m_strModelTag,
        TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
    {
        MSG_BOX("Failed to Add Component : Model in CCharSelect_Pig");
        return E_FAIL;
    }
    return S_OK;
}

CCharSelect_Pig* CCharSelect_Pig::Create(EngineContext* pContext)
{
    CCharSelect_Pig* pInstance = new CCharSelect_Pig(pContext);
    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed to Create : CCharSelect_Pig");
        Safe_Release(pInstance);
    }
    return pInstance;
}

CGameObject* CCharSelect_Pig::Clone(void* pArg)
{
    CCharSelect_Pig* pInstance = new CCharSelect_Pig(*this);
    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Clone : CCharSelect_Pig");
        Safe_Release(pInstance);
    }
    return pInstance;
}

void CCharSelect_Pig::Free()
{
