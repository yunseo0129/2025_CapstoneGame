...
{
    if (_nMaterial < 0 || _nMaterial >= (int)m_iNumMaterials || nullptr == m_Materials[_nMaterial])
        return E_FAIL;
...
// ============================================================================
_uint CModel::Get_MeshNumBones(_uint iMeshIndex) const
{
    if (iMeshIndex >= m_iNumMeshes)
        return 0;
...
const _float4x4* CModel::Get_MeshOffsetMatrices(_uint iMeshIndex) const
{
    if (iMeshIndex >= m_iNumMeshes)
        return nullptr;
    return m_Meshes[iMeshIndex]->Get_BoneOffsetMatrices();
}

HRESULT CModel::Render_Raw(ID3D12GraphicsCommandList* _commandList, _uint iMeshIndex)
{
    if (iMeshIndex >= m_iNumMeshes)
        return E_FAIL;
    // 머티리얼 바인딩 없이 메시만 그린다(diffuse 는 Fracture_System 이 자체 RS 로 바인딩).
    return m_Meshes[iMeshIndex]->Render(_commandList);
}

CTexture* CModel::Get_DiffuseTexture(_uint iMeshIndex) const
{
    if (iMeshIndex >= m_iNumMeshes)
        return nullptr;
    _uint iMaterialIndex = m_Meshes[iMeshIndex]->Get_MaterialIndex();
    if (iMaterialIndex >= m_iNumMaterials)
        return nullptr;
    return m_Materials[iMaterialIndex]->Get_Texture((TextureType)TextureType_DIFFUSE, 0);
}