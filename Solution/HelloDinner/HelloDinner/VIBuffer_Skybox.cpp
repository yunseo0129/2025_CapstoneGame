...
HRESULT CVIBuffer_Skybox::Initialize_Prototype(ID3D12GraphicsCommandList* _commandList)
{
    ...
    indices.push_back(7); indices.push_back(4); indices.push_back(5);
...
CVIBuffer_Skybox* CVIBuffer_Skybox::Create(EngineContext* _pContext)
{
    CVIBuffer_Skybox* pInstance = new CVIBuffer_Skybox(_pContext);
    if (FAILED(pInstance->Initialize_Prototype(_pContext->cmdList)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Create : CVIBuffer_Skybox");
    }
    return pInstance;
}

CComponent* CVIBuffer_Skybox::Clone(void* pArg)
{
    CVIBuffer_Skybox* pInstance = new CVIBuffer_Skybox(*this);
    if (FAILED(pInstance->Initialize(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CVIBuffer_Skybox");
    }
    return pInstance;
}

void CVIBuffer_Skybox::Free()
{
    __super::Free();
}