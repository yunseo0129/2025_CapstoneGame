...
CPlayer_Fish::CPlayer_Fish(EngineContext* _pcontext)
    : CContainerObj{ _pcontext }
{

}

CPlayer_Fish::CPlayer_Fish(const CPlayer_Fish& Prototype)
    : CContainerObj(Prototype.m_pContext)
{

}

HRESULT CPlayer_Fish::Initialize_Prototype()
{
    return S_OK;
}
static int a = 0;
HRESULT CPlayer_Fish::Initialize(void* pArg)
{
    if (nullptr == pArg)
        return E_FAIL;

    PLAYER_FISH_DESC* pDesc = static_cast<PLAYER_FISH_DESC*>(pArg);
...
void CPlayer_Fish::Priority_Update(_float fTimeDelta)
{
    __super::Priority_Update(fTimeDelta);
}

void CPlayer_Fish::Update(_float fTimeDelta)
{
...
void CPlayer_Fish::Late_Update(_float fTimeDelta)
{
    Cull_And_Submit(CRenderer::RG_NONBLEND);
    __super::Late_Update(fTimeDelta);
}

void CPlayer_Fish::Render(ID3D12GraphicsCommandList* _commandList)
{
...
void CPlayer_Fish::ShadowRender(ID3D12GraphicsCommandList* _commandList)
{
...
HRESULT CPlayer_Fish::Ready_PartObjects()
{
    // 케첩건
    {
...
HRESULT CPlayer_Fish::Ready_Components()
{
    // Model 컴포넌트 생성
    if (FAILED(Add_Component(m_iModelLevelIndex, m_strModelTag,
        TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
    {
        MSG_BOX("Failed to Add Component : Model in PLAYER_FISH");
        return E_FAIL;
    }
...
CPlayer_Fish* CPlayer_Fish::Create(EngineContext* _pcontext)
{
    CPlayer_Fish* pInstance = new CPlayer_Fish(_pcontext);
    if (FAILED(pInstance->Initialize_Prototype()))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Create : CPlayer_Fish");
    }
    return pInstance;
}

CGameObject* CPlayer_Fish::Clone(void* pArg)
{
    CPlayer_Fish* pInstance = new CPlayer_Fish(*this);
    if (FAILED(pInstance->Initialize(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CPlayer_Fish");
    }
    return pInstance;
}

void CPlayer_Fish::Free()
{
    __super::Free();
}
