...
public:
	// 셰이더 바인딩 함수
	virtual HRESULT Bind_ShaderResource(ID3D12GraphicsCommandList* pCommandList, _uint iRootParameterIndex, _uint iTextureIndex = 0);
	...
	_uint Get_TextureType() const { return m_eType; }
...
protected:
	...
	_uint						m_eType = RootParameter::Texture2D;

private:
	// 내부 유틸리티 함수
	virtual HRESULT Load_DDSTexture(ID3D12GraphicsCommandList* pCommandList, const wstring& pFilePath, _uint iIndex);
	HRESULT SetResourceDesc(ID3D12Resource* _pTexture, _uint _type);

	virtual HRESULT Create_SrvDescriptorHeap();
	virtual HRESULT CreateShaderResourceView(CD3DX12_CPU_DESCRIPTOR_HANDLE _descriptorHandle, _uint _iIndex, _uint _irootParameterIndex);
...
public:
	virtual HRESULT Initialize_Prototype(ID3D12GraphicsCommandList* pCommandList, const _tchar* pTextureFilePath, _uint iNumTexture, _uint _irootParameterIndex);
	virtual HRESULT Initialize(void* pArg) override;

	static CTexture* Create(ID3D12Device* pDevice, ID3D12GraphicsCommandList* pCommandList, const _tchar* pTextureFilePath, _uint _irootParameterIndex = RootParameter::Texture2D, _uint iNumTextures = 1);
